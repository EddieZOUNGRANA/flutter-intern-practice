...
class TakstodosAction extends StatefulWidget {
  const TakstodosAction({
    super.key,
    required this.text,
    required this.edit,
  ...
  @override
  State<TakstodosAction> createState() => _TakstodosActionState();
}

class _TakstodosActionState extends State<TakstodosAction> {
  ...
  Future<Tasks> getTaskFromTodo(Todos todo) async {
    final snapshot = await FirebaseFirestore.instance
        .collection('tasks')
        .doc(todo.taskId.toString() )
        .get();
    return Tasks.fromDocument(snapshot);
  }

  @override
  initState() {
    if (widget.edit) {
      selectedTask = todoController.tasks.firstWhere(
        (element) => element.title == widget.todo!.taskstitle,
      );
      todoController.textTodoConroller.text = widget.todo!.taskstitle;
  ...
  Future<List<Tasks>> getTaskAll(String pattern) async {
    if (user != null) {
      ...
      final tasks = snapshot.docs.map((doc) => Tasks.fromDocument(doc)).toList();
                ...
                widget.category
                    ? RawAutocomplete<Tasks>(
                        ...
                        onSelected: (Tasks selection) {
                          todoController.textTodoConroller.text =
                              selection.title;
